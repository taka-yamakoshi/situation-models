...
def PlotHeatMap(ax,plot_data,title_text,args):
    im = ax.imshow(plot_data.mean(axis=0))
    fig.colorbar(im, ax=ax,shrink=0.7)
    if args.roll_out:
        ax.set_title(f'{title_text}\nmax of last layer: {plot_data.mean(axis=0).max(axis=-1)[-1]:.5f}')
    else:
        ax.set_title(f'{title_text}\naverage: {plot_data.mean():.5f}')
...
if __name__=='__main__':
    ...
    if args.dataset=='superglue':
        ...
        with open(f'datafile/superglue_wsc_attention{norm_id}_{args.model}_{args.stimuli}{roll_out_id}.pkl','rb') as f:
            attn_data = pickle.load(f)
    elif args.dataset=='winogrande':
        with open(f'datafile/winogrande_{args.size}_prediction_{args.model}.pkl','rb') as f:
            pred_data = pickle.load(f)
        with open(f'datafile/winogrande_{args.size}_attention{norm_id}_{args.model}{roll_out_id}.pkl','rb') as f:
            attn_data = pickle.load(f)
    ...
    if args.verbose:
        print('----------------')
        if args.dataset=='superglue':
            print(f'{args.model} for {args.stimuli}')
        elif args.dataset=='winogrande':
            print(f'{args.model} for {args.size}')
        print(f'\tprediction score: {np.mean(pred_score>0)}')
        print(f'\tmaximum attention score: {np.mean(attn_score>0,axis=0).max()}')
        print(f'\tpaired prediction score: {np.mean(paired_pred_score>0)}')
        print(f'\tstrict paired prediction score: {np.mean(strict_paired_pred_score==1)}')
    ...
    PlotHeatMap(axs[1],paired_attn_score_choices_correct,'overall attention on correct NPs',args)

    PlotLine(axs[2],paired_attn_score_choices_incorrect)
    PlotHeatMap(axs[3],paired_attn_score_choices_incorrect,'overall attention on incorrect NPs',args)

    PlotLine(axs[4],paired_attn_score_context)
    PlotHeatMap(axs[5],paired_attn_score_context,'attention on context words',args)

    PlotLine(axs[6],paired_attn_score_choices)
    PlotHeatMap(axs[7],paired_attn_score_choices,'attention difference',args)

    PlotLine(axs[8],paired_attn_score_period)
    PlotHeatMap(axs[9],paired_attn_score_period,'attention on periods',args)

    plt.tight_layout()
    if args.dataset=='superglue':
        fig.savefig(f'figures/superglue_wsc_attn{norm_id}_all_{args.model}_{args.context_attn_type}_{args.stimuli}{args.input_type}{roll_out_id}.png')
    elif args.dataset=='winogrande':
        fig.savefig(f'figures/winogrande_{args.size}_attn{norm_id}_all_{args.model}_{args.context_attn_type}{args.input_type}{roll_out_id}.png')
    ...
    if args.strict:
        PlotHeatMap(axs[0,0],paired_attn_score_choices_correct[strict_paired_pred_score==1],'overall attention on correct NPs',args)
        axs[0,0].set_ylabel('Correct Prediction')
        PlotHeatMap(axs[0,1],paired_attn_score_choices_incorrect[strict_paired_pred_score==1],'overall attention on incorrect NPs',args)
        PlotHeatMap(axs[0,2],paired_attn_score_context[strict_paired_pred_score==1],'attention on context words',args)
        PlotHeatMap(axs[0,3],paired_attn_score_choices[strict_paired_pred_score==1],'attention difference',args)
        PlotHeatMap(axs[0,4],paired_attn_score_period[strict_paired_pred_score==1],'attention on periods',args)

        PlotHeatMap(axs[1,0],paired_attn_score_choices_correct[strict_paired_pred_score!=1],'overall attention on correct NPs',args)
        axs[1,0].set_ylabel('Incorrect Prediction')
        PlotHeatMap(axs[1,1],paired_attn_score_choices_incorrect[strict_paired_pred_score!=1],'overall attention on incorrect NPs',args)
        PlotHeatMap(axs[1,2],paired_attn_score_context[strict_paired_pred_score!=1],'attention on context words',args)
        PlotHeatMap(axs[1,3],paired_attn_score_choices[strict_paired_pred_score!=1],'attention difference',args)
        PlotHeatMap(axs[1,4],paired_attn_score_period[strict_paired_pred_score!=1],'attention on periods',args)

    else:
        PlotHeatMap(axs[0,0],paired_attn_score_choices_correct[paired_pred_score>0],'overall attention on correct NPs',args)
        axs[0,0].set_ylabel('Correct Prediction')
        PlotHeatMap(axs[0,1],paired_attn_score_choices_incorrect[paired_pred_score>0],'overall attention on incorrect NPs',args)
        PlotHeatMap(axs[0,2],paired_attn_score_context[paired_pred_score>0],'attention on context words',args)
        PlotHeatMap(axs[0,3],paired_attn_score_choices[paired_pred_score>0],'attention difference',args)
        PlotHeatMap(axs[0,4],paired_attn_score_period[paired_pred_score>0],'attention on periods',args)

        PlotHeatMap(axs[1,0],paired_attn_score_choices_correct[paired_pred_score<=0],'overall attention on correct NPs',args)
        axs[1,0].set_ylabel('Incorrect Prediction')
        PlotHeatMap(axs[1,1],paired_attn_score_choices_incorrect[paired_pred_score<=0],'overall attention on incorrect NPs',args)
        PlotHeatMap(axs[1,2],paired_attn_score_context[paired_pred_score<=0],'attention on context words',args)
        PlotHeatMap(axs[1,3],paired_attn_score_choices[paired_pred_score<=0],'attention difference',args)
        PlotHeatMap(axs[1,4],paired_attn_score_period[paired_pred_score<=0],'attention on periods',args)

    plt.tight_layout()
    if args.dataset=='superglue':
        fig.savefig(f'figures/superglue_wsc_attn{norm_id}_all_indiv_{args.model}_{args.context_attn_type}_{args.stimuli}{args.input_type}{roll_out_id}{strict_id}.png')
    elif args.dataset=='winogrande':
        fig.savefig(f'figures/winogrande_{args.size}_attn{norm_id}_all_indiv_{args.model}_{args.context_attn_type}{args.input_type}{roll_out_id}{strict_id}.png')
    ...
    plt.tight_layout()
    if args.dataset=='superglue':
        fig.savefig(f'figures/corr/superglue_wsc_attn{norm_id}_corr_all_{args.model}_{args.context_attn_type}_{args.stimuli}{args.input_type}{roll_out_id}.png')
    elif args.dataset=='winogrande':
        fig.savefig(f'figures/corr/winogrande_{args.size}_attn{norm_id}_corr_all_{args.model}_{args.context_attn_type}{args.input_type}{roll_out_id}.png')
