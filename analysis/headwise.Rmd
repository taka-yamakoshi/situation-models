...
```{r,message=FALSE}
d.zindiv.raw <- read_csv(here('results/combined_effect_ave_z_rep_albert-xxlarge-v2.csv'))
#d.qry.raw <- read_csv(here('results/combined_effect_ave_query_albert-xxlarge-v2.csv'))
d.key.raw <- read_csv(here('results/combined_effect_ave_key_albert-xxlarge-v2.csv'))
d.val.raw <- read_csv(here('results/combined_effect_ave_value_albert-xxlarge-v2.csv'))
```
...
```
```{r}
d.zindiv <- preprocess(d.zindiv.raw) %>% calc_effect()
#d.qry <- preprocess(d.qry.raw) %>% calc_effect()
d.key <- preprocess(d.key.raw) %>% calc_effect()
d.val <- preprocess(d.val.raw) %>% calc_effect()
```
...
ggsave('head-specificity.pdf', units = 'in', width = 5, height = 6)
...
t.vals.all <- d.headall %>%
  ...
  mutate(alpha = 0) %>%
  mutate(alpha = replace(alpha,p.log>-log10(0.05/12/64/5),0.4)) %>%
  mutate(alpha = replace(alpha,p.log>-log10(0.01/12/64/5),0.6)) %>%
  mutate(alpha = replace(alpha,p.log>-log10(0.005/12/64/5),0.8)) %>%
  mutate(alpha = replace(alpha,p.log>-log10(0.001/12/64/5),1.0)) %>%
...
#for (plot_head_id in c(3,8,11,16,17,21,23,24,26,28,29,35,37,40,42,45,46,48,50,51,53,55,56,58,60)) {
  for (plot_cue_type in c("context","verb")) {
    ...
    ggsave(paste0('headwise/all/alpha/head_',plot_head_id,'_',plot_cue_type,'.pdf',sep=""), units = 'in', width = 7.5, height = 3.6)
  }
}
```
...
ggsave(paste('supps/zrep_all_',plot_cue_type,'.pdf',sep=""), units = 'in', width = 13.5, height = 16)
}
```
...
```{r}
d.zindiv.plot <- bind_rows(d.zindiv.boot %>% 
                              select(cue_type,layer_id,head_id,pos_type,y,ci_lower,ci_upper) %>%
                              mutate(plot_type='line'),
                            t.vals.zrep %>%
                              mutate(plot_type='tile'))
```
...
d.zindiv.head %>%
  ggplot(aes(x=as.factor(layer_id), y=y, group=pos_type)) +
  geom_hline(yintercept=0) +
  geom_tile(data = subset(d.zindiv.head, plot_type == 'tile'),
            height=bar_height,width=1,
            #aes(alpha=alpha,color="white",fill=pos_type)) +
            aes(alpha=alpha,fill=pos_type)) +
  geom_line(data = subset(d.zindiv.head, plot_type == 'line'),
            aes(color=pos_type),
            linewidth=0.8,position=position_dodge(width=0.3)) +
  geom_errorbar(data = subset(d.zindiv.head, plot_type == 'line'),
                aes(ymin=ci_lower, ymax=ci_upper, color=pos_type), 
                width=0.5,size=0.5,linetype='solid',position=position_dodge(width=0.3)) +
  theme_few() +
  scale_color_manual(values=c("#E69F00","#D55E00","#56B4E9","#009E73","#000000")) +
  scale_fill_manual(values=c("#E69F00","#D55E00","#56B4E9","#009E73","#000000")) +
  scale_alpha(limits = c(0.0, 1.0)) +
  ...
  labs(x = 'Layer', y ='Effect') +
  guides(color=FALSE,alpha=FALSE,fill=guide_legend(ncol=5,order=1)) +
  facet_grid(plot_type~head_id,scales = "free_y") +
  force_panelsizes(rows = c(unit(0.1,"in"), unit(0.5,"in")))+
  theme(legend.position = 'top', legend.title = element_blank(), legend.box="vertical",
        axis.text.x=element_text(size=10), axis.text.y=element_text(size=14), axis.title=element_text(size=18),
        strip.text.x=element_text(size=16), legend.text=element_text(size=16), strip.text.y=element_blank(),
        axis.line = element_line(colour = "black"),
...
d.val %>%
  ...
  ggplot(aes(x = as.factor(layer_id), y = empirical_stat, color = pos_type, group = pos_type, fill = pos_type)) +
    ...
    theme(aspect.ratio = 1,legend.position = 'top', legend.title = element_blank(), 
          axis.text.x=element_text(size=7),axis.text.y=element_text(size=10), axis.title=element_text(size=16),
          strip.text=element_text(size=10), legend.text=element_text(size=16))
ggsave(paste('supps/value_all_',plot_cue_type,'.pdf',sep=""), units = 'in', width = 13.5, height = 16)
}
```
...
d.qry %>%
  ...
  ggplot(aes(x = as.factor(layer_id), y = empirical_stat, color = pos_type, group = pos_type, fill=pos_type)) +
    ...
    theme(aspect.ratio = 1,legend.position = 'top', legend.title = element_blank(), 
          axis.text.x=element_text(size=7),axis.text.y=element_text(size=10), axis.title=element_text(size=16),
          strip.text=element_text(size=10), legend.text=element_text(size=16))
ggsave(paste('supps/query_all_',plot_cue_type,'.pdf',sep=""), units = 'in', width = 13.5, height = 16)
}
```
...
d.key %>%
  ...
  ggplot(aes(x = as.factor(layer_id), y = empirical_stat, color = pos_type, group = pos_type, fill=pos_type)) +
    ...
    theme(aspect.ratio = 1,legend.position = 'top', legend.title = element_blank(), 
          axis.text.x=element_text(size=7),axis.text.y=element_text(size=10), axis.title=element_text(size=16),
          strip.text=element_text(size=10), legend.text=element_text(size=16))
ggsave(paste('supps/key_all_',plot_cue_type,'.pdf',sep=""), units = 'in', width = 13.5, height = 16)
}
```