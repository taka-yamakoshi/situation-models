...
d.headall.max.boot <- d.headall.max %>%
  ...
  tidyboot_mean(effect_ave, nboot = 10) %>%
  rename(y=empirical_stat)
write.csv(d.head_all.boot,file=str_glue("headall_max_boot_{model_name}.csv"),row.names=FALSE)
d.head_all.boot <- read_csv(str_glue("headall_max_boot_{model_name}.csv"))
```
