...
d.zindiv.raw %>% 
  ...
  group_by(cue_type) %>%
  summarize(ratio = mean(acc), num = sum(acc), total = sum(acc>-1))
```
...
```{r,message=FALSE}
d.headall.boot <- d.headall %>%
  group_by(cue_type,layer_id,head_id,rep_type,pos_type) %>%
  tidyboot_mean(effect_ave, nboot = 10) %>%
  rename(y=empirical_stat)
write.csv(d.headall.boot,file=str_glue("headall_boot_{model_name}.csv"),row.names=FALSE)
#d.headall.boot <- read_csv(str_glue("headall_boot_{model_name}.csv"))
```
...
```{r}
d.headall.plot <- bind_rows(d.headall.boot %>% 
...
```{r}
d.headall.max <- d.headall %>%
    filter(pos_type%in%pos_type_list) %>%
    mutate(pos_type=factor(pos_type,levels=pos_type_list)) %>%
    group_by(cue_type, rep_type, pos_type, layer_id, head_id) %>%
    mutate(mean_val = mean(effect_ave)) %>%
    group_by(cue_type, rep_type, pos_type, layer_id, pair_id) %>%
    slice(which.max(mean_val))
```

```{r,message=FALSE}
d.headall.max.boot <- d.headall.max %>%
  group_by(cue_type,rep_type,pos_type,layer_id) %>%
  tidyboot_mean(effect_ave, nboot = 100) %>%
  rename(y=empirical_stat)
write.csv(d.headall.max.boot,file=str_glue("headall_max_boot_{model_name}.csv"),row.names=FALSE)
#d.headall.max.boot <- read_csv(str_glue("headall_max_boot_{model_name}.csv"))
```

```{r}
bar_center <- -0.01
bar_height <- 0.02
t.vals.max.all <- d.headall.max %>%
  group_by(cue_type, rep_type, pos_type, layer_id) %>%
  summarize(p.val = t.test(effect_ave)$p.value,
            t.val = t.test(effect_ave)$statistic) %>%
  mutate(p.log=(-log10(p.val))) %>%
  ...
  mutate(alpha = replace(alpha,p.log>-log10(0.005/12/64/5),0.6)) %>%
  mutate(alpha = replace(alpha,p.log>-log10(0.001/12/64/5),0.8)) %>%
  select(cue_type,layer_id,rep_type,pos_type,y,alpha,p.val)
write.csv(t.vals.max.all,file=str_glue("tvals_all_max_{model_name}.csv"),row.names=FALSE)
```

```{r}
d.headall.max.plot <- bind_rows(d.headall.max.boot %>% 
                              select(cue_type,layer_id,rep_type,pos_type,y,ci_lower,ci_upper) %>%
                              mutate(plot_type='line'),
                            t.vals.max.all %>%
                              mutate(plot_type='tile'))
```
...
for (plot_cue_type in c("context","verb")) {
    ...
    d.head %>%
      ...
      theme(legend.position = 'top', legend.title = element_blank(), legend.box="vertical",
            ...
            axis.line = element_line(colour = "black"),
            panel.border = element_blank(),panel.background = element_blank())
    ggsave(str_glue('{model_name}-{plot_cue_type}.pdf',sep=""), units = 'in', width = 7.5, height = 3.6)
}
```

```{r}
d.headall.max.stats <- d.headall.max %>%
  select(cue_type, rep_type, pos_type, layer_id, pair_id, effect_ave) %>%
  pivot_wider(names_from = pos_type, values_from = effect_ave) %>%
  group_by(cue_type, rep_type, layer_id) %>%
  summarize(options_p.val = t.test(options)$p.value,
            ...
            context_p.val = t.test(context)$p.value,
            verb_p.val = t.test(verb)$p.value,
            masks_p.val = t.test(masks)$p.value,
            rest_p.val = t.test(rest)$p.value)
```
```{r}
d.headall.max.stats %>% filter()
```

```{r}
d.headall.tmax <- d.headall %>%
    filter(pos_type%in%pos_type_list) %>%
    mutate(pos_type=factor(pos_type,levels=pos_type_list)) %>%
    group_by(cue_type, rep_type, pos_type, layer_id, head_id) %>%
    mutate(t.val = t.test(effect_ave)$statistic) %>%
    group_by(cue_type, rep_type, pos_type, layer_id, pair_id) %>%
    slice(which.max(t.val))
```

```{r,message=FALSE}
d.headall.tmax.boot <- d.headall.tmax %>%
  group_by(cue_type,rep_type,pos_type,layer_id) %>%
  tidyboot_mean(effect_ave, nboot = 100) %>%
  rename(y=empirical_stat)
write.csv(d.headall.tmax.boot,file=str_glue("headall_tmax_boot_{model_name}.csv"),row.names=FALSE)
#d.headall.max.boot <- read_csv(str_glue("headall_max_boot_{model_name}.csv"))
```
...
bar_center <- -0.01
bar_height <- 0.02
t.vals.tmax.all <- d.headall.tmax %>%
  group_by(cue_type, rep_type, pos_type, layer_id) %>%
  summarize(p.val = t.test(effect_ave)$p.value,
            t.val = t.test(effect_ave)$statistic) %>%
  mutate(p.log=(-log10(p.val))) %>%
  ...
  mutate(alpha = replace(alpha,p.log>-log10(0.001/12/64/5),0.8)) %>%
  select(cue_type,layer_id,rep_type,pos_type,y,alpha,p.val)
write.csv(t.vals.tmax.all,file=str_glue("tvals_all_tmax_{model_name}.csv"),row.names=FALSE)
```

```{r}
d.headall.tmax.plot <- bind_rows(d.headall.tmax.boot %>% 
                              select(cue_type,layer_id,rep_type,pos_type,y,ci_lower,ci_upper) %>%
                              mutate(plot_type='line'),
                            t.vals.tmax.all %>%
                              mutate(plot_type='tile'))
...
for (plot_cue_type in c("context","verb")) {
    ...
    d.head %>%
      ...
      guides(color=FALSE,alpha=FALSE,fill=guide_legend(ncol=5,order=1)) +
      facet_grid(plot_type~rep_type,scales = "free_y") +
      force_panelsizes(rows = c(unit(0.3,"in"), unit(1.5,"in")),
                       cols = c(unit(3,"in"),unit(3,"in"), unit(3,"in"))) +
      theme(legend.position = 'top', legend.title = element_blank(), legend.box="vertical",aspect.ratio = 0.5,
            axis.text.x=element_text(size=10), axis.text.y=element_text(size=14), axis.title=element_text(size=18),
            strip.text.x=element_text(size=16), legend.text=element_text(size=16), strip.text.y=element_blank(),
            axis.line = element_line(colour = "black"),
            panel.border = element_blank(),panel.background = element_blank())
    ggsave(str_glue('{model_name}-{plot_cue_type}-tmax.pdf',sep=""), units = 'in', width = 12, height = 4.4)
}
```
