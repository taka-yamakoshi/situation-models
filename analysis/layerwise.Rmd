...
```{r,message=FALSE}
d.layer.raw <- read_csv(here('dataset/combined_effect_ave_layer-query-key-value_multihead_albert-xxlarge-v2.csv')) %>%
...
sent_list <- d.layer.raw %>%
  ...
  mutate(group_score=str_c(context,verb,context_verb,sep="-")) %>% 
  filter(group_score=="TRUE-TRUE-TRUE") %>%
  pull("pair_id")
```
